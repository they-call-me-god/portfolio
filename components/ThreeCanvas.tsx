...
// ── Particle field — 280 points in a sphere, slow axial rotation + scroll drift
...
// ── Floating wireframe icosahedra at various Z depths — the real 3D depth layer
function FloatingIcosahedra() {
  ...
  useFrame(({ clock }) => {
    ...
    meshRefs.current.forEach((mesh, i) => {
      ...
      // Gentle bob at unique frequencies
...
// ── Deep background glow orbs — large, very transparent, breathing
function GlowOrbs() {
  const groupRef = useRef<THREE.Group>(null)
...
// ── Camera rig — smooth mouse parallax + subtle scroll tilt
function CameraRig() {
...
// ── Exported component — SSR-safe (always imported with { ssr: false })
